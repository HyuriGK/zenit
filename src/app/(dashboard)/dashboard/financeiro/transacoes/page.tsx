  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden bg-zinc-950 p-4 lg:p-6 space-y-4 sm:space-y-6">
      ...
      {carregando ? (
        ...
        {visualizacao === 'planilha' && <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-zinc-800 bg-zinc-800 sm:grid-cols-3"><div className="bg-zinc-950 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Soma automática</p><p className="mt-1 text-lg font-black text-zinc-100">{formatarMoeda(transacoesFiltradas.reduce((total, t) => total + t.valor, 0))}</p></div><div className="bg-zinc-950 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Restante provisionado</p><p className="mt-1 text-lg font-black text-amber-300">{formatarMoeda(transacoesFiltradas.reduce((total, t) => total + (t.isParcela && t.parcelaTotais && t.parcelaNumero ? t.valor * (t.parcelaTotais - t.parcelaNumero + 1) : 0), 0))}</p></div><div className="bg-zinc-950 px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Quitação provisionada</p><p className="mt-1 text-lg font-black text-cyan-300">{formatarMoeda(transacoesFiltradas.filter((t) => t.isParcela).reduce((total, t) => total + t.valor, 0))}</p></div></div>}
